...
def professional_lead(topic: str, categories: list[str], text: str) -> str:
    ...
    if "health" in categories or has_term(text, ("glucose", "biosensor", "metabolic", "medical", "health", "clinical", "patient", "treatment")):
        ...
        return f"Health technology can be useful, but it becomes risky when people mistake consumer sensors for medical devices. The key is understanding what the data actually shows and what it doesn't."

    if has_term(text, ("robotaxi", "robotaxis", "self-driving", "autonomous", "driverless", "tesla")):

        return "Self-driving technology has moved from polished demos to messy reality on public roads. The real test isn't whether it works sometimes, but whether it works reliably enough for regulators and passengers to trust it."

    if has_term(text, ("spacex", "dragon", "space station", "resupply", "station", "nasa")):

        return "Space missions matter less for the spectacle and more for what they enable: experiments, data, and research that can't be done on Earth."

    if "chatgpt" in text and "malta" in text:

        return "OpenAI's partnership with Malta turns ChatGPT Plus into something closer to public infrastructure, making AI access available to all citizens rather than just those who can pay for it."

    if "iphone" in text and "china" in text and any(word in text for word in ("discount", "price", "slash", "618")):

        return "Apple's iPhone discounts in China signal that even premium brands are feeling competitive pressure. Price cuts ahead of major shopping seasons suggest demand isn't as automatic as it used to be."

    if has_term(text, ("residual stress", "aircraft", "engine blades", "materials", "reliability", "manufacturing")):

        return "Better ways to measure stress in aircraft parts could mean safer planes and fewer failures. This is the kind of engineering work that matters more than it sounds."

    if "science" in categories or has_term(text, ("research", "study", "scientists", "experiment", "breakthrough", "discovery")):

        return "Research matters when it gives scientists better tools or clearer answers. The question is whether this work actually advances understanding or just adds noise."

    if has_term(text, ("chatgpt", "openai", "ai", "artificial intelligence", "gemini")):

        return "AI features matter when they make software actually better, not just more decorated. The real test is whether they improve real work, not just demos."

    if "apple" in categories:

        return "Apple news matters because it rarely stays inside one product. Changes can affect the entire ecosystem of users, developers, and accessories."

    if "phones" in categories:

        return "Phone news matters when it affects real decisions: pricing, upgrade timing, software support, and whether the changes are worth the cost."

    return "Tech news matters when it actually changes what people can do, not just what companies announce."
...
def editorial_nut_graph(categories: list[str], text: str) -> str:
    ...
    if kind == "health":
        ...
        return "Health technology needs to balance usefulness with honesty. The real test is whether companies are clear about what their sensors can and cannot do, rather than turning uncertain data into false certainty."

    if kind == "science":

        return "The most important science isn't always the most dramatic. What matters is whether the evidence is strong, the limits are clear, and the work gives other researchers a better foundation for the next step."

    if kind == "space":

        return "Space missions matter less for the spectacle and more for what they enable: better data, tougher materials, and new ways to solve problems back on Earth."

    if kind == "ai":

        return "AI has moved past the novelty phase. The real question now is whether it actually makes software better or just more decorated. Reliability, privacy, and usefulness matter more than impressive demos."

    if kind == "phones":

        return "Small changes in phones can matter more than big ones. A better sensor, longer support, or smarter software can genuinely improve daily life, not just look good on a spec sheet."

    if kind == "security":

        return "Security issues are easy to ignore until they become personal. The smart approach is to treat them as early warnings and patch early rather than waiting for a problem."

    if kind == "gadgets":

        return "Good gadgets solve real problems elegantly. The ones that fail usually do so on boring details: price, battery life, reliability, and whether anyone actually wants to use them more than once."

    return "The real test of any technology news isn't how interesting it sounds today, but whether it actually changes how people live, work, or make decisions."
...
def upload_media_multipart(path: Path, title: str) -> int | None:
    """Upload a local image to the wp.com media library via the v1.1 multipart
    endpoint. The JSON REST media route on WordPress.com rejects missing
    Content-Disposition, so real uploads need this form-data path."""
    import io
    ...
    stream.write(b"Content-Type: image/png\r\n\r\n")
    stream.write(raw)
    stream.write(f"\r\n--{boundary}--\r\n".encode())
    request = urllib.request.Request(
    ...
    if isinstance(data, dict) and data.get("media"):
        return int(data["media"][0]["ID"])
    return None
    ...
    if hero_path:
        try:
            featured_media_id = upload_media_multipart(
                hero_path, article.get("hero_image_alt") or article.get("title", "")
            )
